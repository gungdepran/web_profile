...
<head>
    ...
    <title>Admin Panel - Edit Regulasi</title>
...
<div class="sidebar d-flex flex-column flex-shrink-0 p-3 text-white bg-dark">
    {{-- Kode sidebar sama seperti halaman index --}}
    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-header bg-primary text-white">
                <h1 class="card-title mb-0">Edit Regulasi</h1>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.regulasi.update', $regulasi->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('judul') is-invalid @enderror" id="judul" name="judul" value="{{ old('judul', $regulasi->judul) }}" required>
                        @error('judul')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="mb-3">
                        <label for="tanggal" class="form-label">Tanggal</label>
                        <input type="date" class="form-control @error('tanggal') is-invalid @enderror" id="tanggal" name="tanggal" value="{{ old('tanggal', $regulasi->tanggal) }}" required>
                        @error('tanggal')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="mb-3">
                        <label for="isi_keputusan" class="form-label">Isi Keputusan</label>
                        <textarea class="form-control @error('isi_keputusan') is-invalid @enderror" id="isi_keputusan" name="isi_keputusan" rows="5" required>{{ old('isi_keputusan', $regulasi->isi_keputusan) }}</textarea>
                        @error('isi_keputusan')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="mb-3">
                        <label for="thumbnail" class="form-label">Thumbnail (Ganti jika perlu)</label>
                        @if($regulasi->thumbnail)
                            <img src="{{ asset('storage/' . $regulasi->thumbnail) }}" alt="Thumbnail" width="150" class="mb-2 d-block">
                        @endif
                        <input type="file" class="form-control @error('thumbnail') is-invalid @enderror" id="thumbnail" name="thumbnail" accept="image/*">
                        @error('thumbnail')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="mb-3">
                        <label for="file_path" class="form-label">File Dokumen (Ganti jika perlu)</label>
                        @if($regulasi->file_path)
                            <p>File saat ini: <a href="{{ asset('storage/' . $regulasi->file_path) }}" target="_blank">Lihat Dokumen</a></p>
                        @endif
                    ...
                    <button type="submit" class="btn btn-primary"><i class="bi bi-save-fill"></i> Update</button>
